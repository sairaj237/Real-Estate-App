...
export default function CreateListing() {
  ...
  const handleImageSubmit = async (e) => {
    if (files.length > 0 && files.length + formData.imageUrls.length < 7) {
      ...
      try {
        const authCheck = await fetch(`${import.meta.env.VITE_API_URL}/api/user/check-auth`, {
          credentials: 'include'
        });
        
        if (!authCheck.ok) {
          setImageUploadError('Your session has expired. Please sign in again.');
          setTimeout(() => {
            navigate('/sign-in');
          }, 2000);
          return;
        ...
        setImageUploadError('Authentication check failed. Please try again.');
        return;
      }
